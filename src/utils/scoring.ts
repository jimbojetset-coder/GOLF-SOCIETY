...
export function calcStrokesReceived(
  ...
  switch (format) {
    ...
    case 'fourball': {
      ...
      return playingHandicaps.map(h => Math.round((h - min) * 0.75));
    }

    case 'foursomes': {
      // Team handicap = (sum of both) / 2, then 3/4 applied
      // playingHandicaps = [teamA_combined, teamB_combined]
      const teamHandicaps = playingHandicaps.map(h => Math.round(h * 0.75));
